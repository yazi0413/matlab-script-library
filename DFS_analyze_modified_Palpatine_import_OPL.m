...
%       tit = title(legend_labels_front{R});  
      tit = title('msg_dfs_off vs freq response without system cal');
      fig = gcf;
...
xlabel('Frequency [Hz]')
ylabel('Magnitude[dB]')
title('Gap between freq resp and calibrated response')  
...
figure(4);
ylim([-100,0])
legend([h_f,h_f1],'Front OPL','Atten curves of FB');
% make the figure full-screen
set(gcf,'units','normalized','position',[0,0,1,1]);
grid on;
...
figure(5);
ylim([-100,0])
legend([h_r,h_r1],'Rear OPL','Atten curves of FB');
grid on;
% make the figure full-screen
set(gcf,'units','normalized','position',[0,0,1,1]);
